...
# sound load
cuesound = pygame.mixer.Sound(os.path.join(assets_path, 'cuesound.wav'))
ballsounds = []
for i in range(5):
...
cushions = [
  ...
  [(630, 101), (642, 117), (1038, 117), (1046, 101)], # upper right
...
def ball_collision(space, arbiter, a):
...
def main():
    ...
    c_handler.begin = ball_collision
    ...
    while not done:
        ...
        for i, ball in enumerate(balls):
            ...
            if ball.body.position[0] < 0 or ball.body.position[0] > WINDOW_WIDTH:
                ball.body.position = (888, WINDOW_HEIGHT / 2)
            if ball.body.position[1] < 0 or ball.body.position[1] > WINDOW_HEIGHT:
                ball.body.position = (888, WINDOW_HEIGHT / 2)
